...
- **固定日期工具栏**：日期、翻页、今天、1/3/7 日和选择入口始终停留在可见区域顶部。
...
- **批量调整**：进入“选择”模式后可多选时间块，统一后移 15/30 分钟、复制或按最早一项定位整体移动；任一目标冲突或越界时整组不保存。
...
自动化测试覆盖内容三态迁移、主题颜色、小时格坐标、整组移动/复制、全天边界、日期范围、重复例外和 JSON 备份兼容。手动验收步骤见 `TEST.md`。
